...
Transform user-selected papers into Obsidian literature notes and a small set of linked terminology notes. Treat messy PDF filenames as input hints only; derive consistent literature-note metadata and filenames from the paper metadata or paper text whenever possible. A Zotero collection is a candidate pool by default, not an instruction to process every item in it.
...
- A Zotero collection name or key, such as `PhD P2 - PEDro`, when `zotero-readonly` MCP tools are available. A collection alone means "inspect or preview this candidate pool" unless the user explicitly asks to process all unprocessed items.
...
## Zotero Selection Policy

Treat Zotero collections as broad candidate pools because the user may store many possible citations that they do not intend to read immediately.

Default behavior:

- If the user names a Zotero collection but does not specify items, titles, tags, or an explicit batch instruction, do not create literature notes or term notes.
- Instead, provide a concise processing preview: item title, year, first author, PDF availability, whether a matching Obsidian literature note already exists, and the exact wording the user can use to process selected items.
- If the collection is large, summarize counts first and show a short shortlist rather than flooding the response.

Process only these Zotero items:

- Items explicitly named by Zotero key, title, DOI, PMID, author-year hint, or an unambiguous numbered selection from a preview.
- Items matching an explicit user-provided filter, such as a Zotero tag, subcollection, search string, or "the three papers I just listed".
- All currently unprocessed items only when the user clearly says something like "处理全部未处理文献", "process all unprocessed items", "把这个 collection 里的未处理项全部做完", or equivalent.

When the user's intent is ambiguous, prefer a preview and ask for the specific items rather than silently processing the whole collection. Do not infer "read all" from collection membership alone.
...
3. If the user names a Zotero collection, use `zotero_collection_snapshot` to get compact metadata and PDF attachment paths.
   - If the user specified selected items or an explicit batch filter, process only matching requested items.
   - If the user explicitly asked to process all unprocessed items, process all items not already represented in Obsidian.
   - If the user only named the collection, stop after a processing preview; do not create literature notes, term notes, or topic entries yet.
